...
export default function CategorySeaFood() {
  const [mealSeaFood, setMealSeaFood] = useState([]);
  const { categories } = useContext(CategoryContext);
  const [search, setSearch] = useState("");

  const url = "https://www.themealdb.com/api/json/v1/1/filter.php?c=Seafood";
  //Funcion para obtener todos los platillos de seafood
  const showData = async () => {
    fetch(url)
      ...
      .then((data) => {
        ...
        */
        setMealSeaFood((prevMeals) => [...prevMeals, ...meals]);
      });
  };

  //Funcion para filtrar las listas de comida
  const filteredMeals = mealSeaFood.filter((meal) =>
