...
export default function CategoryChicken() {
  const [mealChicken, setMealChicken] = useState([]);
  const { categories } = useContext(CategoryContext);
  const [search, setSearch] = useState("");

  const url = "https://www.themealdb.com/api/json/v1/1/filter.php?c=Chicken";
  //Funcion para obtener todos los platillos de chicken
  const showData = async () => {
    fetch(url)
      ...
      .then((data) => {
        ...
        */
        setMealChicken((prevMeals) => [...prevMeals, ...meals]);
      });
  };

  //Funcion para filtrar las listas de comida
  const filteredMeals = mealChicken.filter((meal) =>
