...
export default function CategoryMiscellaneous() {
  const [mealMiscellaneous, setMealMiscellaneous] = useState([]);
  const { categories } = useContext(CategoryContext);
  const [search, setSearch] = useState("");

  const url = "https://www.themealdb.com/api/json/v1/1/filter.php?c=Miscellaneous";
  //Funcion para obtener todos los platillos de misellaneous
  const showData = async () => {
    fetch(url)
      ...
      .then((data) => {
        ...
        */
        setMealMiscellaneous((prevMeals) => [...prevMeals, ...meals]);
      });
  };
  //Funcion para filtrar las listas de comida
  const filteredMeals = mealMiscellaneous.filter((meal) =>
