...
export default function CategoryVegan() {
  const [mealVegan, setMealVegan] = useState([]);
  const { categories } = useContext(CategoryContext);
  const [search, setSearch] = useState("");

  const url = "https://www.themealdb.com/api/json/v1/1/filter.php?c=Vegan";
  //Funcion para obtener todos los platillos de food vegan
  const showData = async () => {
    fetch(url)
      ...
      .then((data) => {
        ...
        */
        setMealVegan((prevMeals) => [...prevMeals, ...meals]);
      });
  };

  //Funcion para filtrar las listas de comida
  const filteredMeals = mealVegan.filter((meal) =>
