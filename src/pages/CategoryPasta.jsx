...
export default function CategoryPasta() {
  const [mealPasta, setMealPasta] = useState([]);
  const { categories } = useContext(CategoryContext);
  const [search, setSearch] = useState("");

  const url = "https://www.themealdb.com/api/json/v1/1/filter.php?c=Pasta";
  //Funcion para obtener todos los platillos de  pasta
  const showData = async () => {
    fetch(url)
      ...
      .then((data) => {
        ...
        */
        setMealPasta((prevMeals) => [...prevMeals, ...meals]);
      });
  };

  //Funcion para filtrar las listas de comida
  const filteredMeals = mealPasta.filter((meal) =>
